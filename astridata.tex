...
The light produced by the FOC is uniform and its intensity provide an average of few $p.e.$ per pixel. 
Data used for the gain evaluation in the oservation runs used for the analyis are listed in Table~\ref{tab:FOC}, they are relative to a fixed voltage $V_{0}$= 57 V \tem{di che?} and a temperature of 15$^\circ$C for a camera pixel. For these acquisition HG data are available. Figure~\ref{fig:FOC} shows the HG pulse height spectrum acquired in the FOC run \tem{quale?}  . 
