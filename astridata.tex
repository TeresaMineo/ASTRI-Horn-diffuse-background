...
ASTRI-Horn camera is equipped with a thermal control system to keep the temperature of the focal plane in the  range required (13-17$^\circ$) to limit the gain variations to a few ADCs. In addition, a dedicated procedure is forseen to evaluate off-line the gain relative to each observation with an accuracy of ...{\tem mettere il valore} ADC. ASTRI-Horn is, in fact, equipped with a side emitting fiber optics system (FOC) located along the PMMA window circumference and connected to a controlled intensity light emitting diode (LED). FOC data are collected periodically during the normal telescope acquisition mode.
The light produced by the FOC is uniform and its intensity provide on average a few $p.e.$ per pixel. 

FOC data used for this paper data analyis are listed in Table~\ref{tab:FOC}; they are relative to a fixed voltage $V_{0}$= 57 V {\tem di che?} and a temperature of 15$^\circ$C {\che vuol dire?}. For these acquisitions HG data are available. Figure~\ref{fig:FOC} shows the HG pulse height spectrum acquired in the FOC run \tem{quale?}. 

The distribution shown is the reconstructed maximum amplitude for all the recorded waveforms of a physical pixel. While the first peak is the pedestal, the following  ones are the single and multiple p.e. contributions. The distance between 
