...
The HG pulse hight distribution for each run listed in Table~\ref{tab:FOC} is accumulated and, for each pixel, the position of the maximum amplitude of each peak is computed. Figure~\ref{fig:FOC} shows the pulse height spectrum relative to pixel ... in the FOC run ...\tem{quale?}. The gain is computed comparing the average difference between two consecutive peak, $p.e._{eq}$ centers with that produced by a reference gain:
